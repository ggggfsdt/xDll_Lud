...
<head>
    ...
    <style>
        /* ─── Full styles from the last working version ─── */
        * {
            margin: 0;
            padding: 0;
        ...
        /* ─── BET OVERLAY with background image (lighter) ─── */
        #bet-overlay-wrap {
            position: absolute;
            bottom: 12px;
            left: 50%;
        ...
        #ice-canvas {
            ...
            touch-action: none;
            display: block;
            box-shadow: inset 0 0 80px rgba(0, 0, 0, 0.9), 0 0 50px rgba(91, 141, 239, 0.06);
            flex-shrink: 0;
            border: 1px solid rgba(255, 255, 255, 0.04);
        }
        #ice-timer-wrap {
        ...
        #ice-timer-progress-wrap.visible {
            display: block;
        }
        #ice-timer-progress {
            height: 100%;
        ...
        #ice-win-history::-webkit-scrollbar {
            display: none;
        }
        ...
        #ice-stats-bar .stat-item {
            display: flex;
            align-items: center;
            gap: 6px;
        }
        #ice-stats-bar .stat-item .icon {
            width: 20px;
            height: 20px;
            object-fit: contain;
        }
        #ice-stats-bar .stat-item .value {
            color: #fff;
            font-weight: 700;
        }
        #ice-stats-bar .stat-item .label {
            color: var(--text2);
            font-size: 12px;
            margin-right: 2px;
        }
...
<body>
    <div id="app">
        ...
        <div id="main">
                ...
                <div id="arena-wrap">
                    ...
                    <div id="auto-bet-container" class="auto-bet-container">
                        ...
                        <div id="auto-bet-panel" class="auto-bet-panel">
                            <div class="row">
                                <label>Amount</label>
                                <input type="text" id="auto-bet-amount" placeholder="10" inputmode="numeric" />
                            </div>
                            <div class="row">
                                <label>Auto</label>
                                <div class="switch-sm" id="auto-switch-wrap">
                                    <input type="checkbox" id="auto-bet-switch" />
            ...
            <div id="ice-page" class="tab-page">
                <div id="ice-arena-wrap">
                    ...
                    <div id="ice-timer-wrap">
                        <div id="ice-countdown-timer">10.00</div>
                        <div id="ice-timer-progress-wrap"><div id="ice-timer-progress"></div></div>
                    </div>
                    <div id="ice-win-history"></div>
                    ...
                    <div id="ice-auto-bet-container" class="ice-auto-bet-container">
                        ...
                        <div id="ice-auto-bet-panel" class="ice-auto-bet-panel">
                            <div class="row">
                                <label>Amount</label>
                                <input type="text" id="ice-auto-bet-amount" placeholder="10" inputmode="numeric" />
                            </div>
                            <div class="row">
                                <label>Auto</label>
                                <div class="switch-sm" id="ice-auto-switch-wrap">
                                    <input type="checkbox" id="ice-auto-bet-switch" />
                                    <span class="slider-sm"></span>
                                </div>
                            </div>
                        </div>
                ...
                <div id="ice-stats-bar">
                    <div class="stat-item"><span class="label">Players</span><span class="value" id="ice-stats-players">0</span></div>
                    <div class="stat-item"><img src="https://i.postimg.cc/Dz2dD6rz/ezgif-2d1a6da23d7bb12a.gif" class="icon" alt="⭐" /><span class="value" id="ice-stats-pot">0</span></div>
                </div>
            </div>
        ...
        <div id="tabs">
            ...
            <button class="tab-btn" data-tab="ice">
                <span class="tab-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <script>
        ...
        const tabPages = {
            ...
            ice: document.getElementById('ice-page'),
        ...
        const iceState = {
            ...
            winnerId: null,
            winHistory: [],
            launchTime: 0, // for pop animation
        };
        ...
        function getIcePlayerLocal(id) {
            return iceState.players.find(p => p.id === id);
        }
        ...
        if (!socket) {
            ...
            socket.on('connect', () => {
                ...
                socket.emit('join', { initData }, (res) => {
                    ...
                    if (res.iceArena) {
                        iceState.perimeterPoints = res.iceArena.perimeter;
                    ...
                    if (Array.isArray(res.iceRecentWinners) && res.iceRecentWinners.length) {
                        iceState.winHistory = res.iceRecentWinners.slice(0, 8).map(w => ({
                            name: w.name,
                            pfp: w.pfp,
                        }));
            ...
            socket.on('iceState', (snapshot) => {
                applyIceServerState(snapshot);
            });

            socket.on('iceRoundEnd', (payload) => {
                handleIceRoundEnd(payload);
            });
        }
        ...
        function applyIceServerState(snapshot) {
            ...
            // Record launch time for pop animation
            if (prevGameState !== 'sliding' && iceState.gameState === 'sliding') {
                iceState.launchTime = Date.now();
            }

            const seen = new Set();
            snapshot.players.forEach(sp => {
                seen.add(sp.id);
                let local = getIcePlayerLocal(sp.id);
                if (!local) {
                ...
                local.bet = sp.bet;
                local.color = sp.color;
                local.segStart = sp.segStart;
                local.segEnd = sp.segEnd;
            });
            ...
            iceTimerWrap.style.display = (iceState.gameState === 'countdown' || iceState.gameState === 'spinning') ? 'flex' :
                iceTimerWrap.style.display;
        ...
        function updateIceWaitingText() {
            ...
            if (n === 0) {
                iceWaitingText.innerHTML =
                '<span>waiting for players</span><div class="sub">place a bet to claim the rink</div>';
                iceWaitingText.style.display = 'block';
            } else if (iceState.gameState === 'idle' || iceState.gameState === 'countdown') {
                iceWaitingText.innerHTML = `<span>● ${n} player${n > 1 ? 's' : ''} on the rink</span>`;
                iceWaitingText.style.display = 'block';
            } else {
        ...
        function refreshLeaderboard() {
            ...
            socket.emit('leaderboard', {}, (res) => {
                ...
                state.topPlayers.forEach(p => {
                    state.topPlayerIds.set(p.id, { rank: p.rank });
                });
                updateTopUI();
            });
        }
        ...
        function setupInputWithClear(inputEl, defaultVal = 10) {
            ...
            inputEl.addEventListener('blur', () => {
                if (inputEl.dataset.empty === 'true' || inputEl.value === '') {
        ...
        function toggleAutoBet(enable) {
            if (enable && !autoBetEnabled) {
                ...
                autoBetInterval = setInterval(() => {
                    const canBet = (state.gameState === 'idle' || state.gameState === 'countdown' || state.gameState ===
                        'prestart');
                    if (!canBet) return;
        ...
        function toggleIceAutoBet(enable) {
            if (enable && !iceAutoBetEnabled) {
                ...
                iceAutoBetInterval = setInterval(() => {
                    ...
                    if (amount < 10 || amount > state.balance) return;
                    socket.emit('icePlaceBet', { amount }, (res) => {
                        if (res?.ok) {
                            state.balance = res.balance;
                            updateBalance();
                            updateProfile();
                ...
                iceAutoToggle.textContent = '⏱';
                if (iceAutoBetInterval) {
                    clearInterval(iceAutoBetInterval);
        ...
        function updateBetUI() {
            ...
            if (betInput.dataset.empty === 'true') {
                betInput.value = 10;
        ...
        function renderLoop() {
            ...
            if (iceState.gameState === 'countdown' && iceState.countdownStartTime) {
                const elapsed = (Date.now() - iceState.countdownStartTime) / 1000;
                const remaining = Math.max(0, 10.0 - elapsed);
                const secs = Math.floor(remaining);
                const ms = Math.floor((remaining - secs) * 100);
                iceCountdownTimer.textContent = secs + '.' + String(ms).padStart(2, '0');
                const progress = Math.min(1, elapsed / 10.0) * 100;
                iceTimerProgress.style.width = progress + '%';
                iceTimerProgressWrap.classList.add('visible');
                iceTimerWrap.style.display = 'flex';
            } else if (iceState.gameState === 'spinning') {
                iceCountdownTimer.textContent = '🌀';
                iceTimerProgress.style.width = '100%';
                iceTimerWrap.style.display = 'flex';
            } else if (iceState.gameState === 'idle') {
                iceTimerProgressWrap.classList.remove('visible');
                iceTimerWrap.style.display = 'none';
            }
        ...
        function placeIceBet(amount) {
            ...
            socket.emit('icePlaceBet', { amount: addAmount }, (res) => {
                if (!res?.ok) {
                    haptic('light');
                    console.warn('Ice bet rejected:', res?.error);
                    return;
                }
                state.balance = res.balance;
                updateBalance();
        ...
        function drawIceArrow(ctx, cx, cy, angle, size) {
            // Arrow pointing in direction 'angle' (0 = right) – orbits around centre
            ctx.save();
            ctx.translate(cx, cy);
            ctx.rotate(angle);
            ctx.beginPath();
            ...
            ctx.shadowColor = 'rgba(139, 197, 255, 0.9)';
            ctx.shadowBlur = 16;
            ctx.fill();
        ...
        function renderIce() {
            ...
            // Camera zoom & track
            const speed = Math.sqrt(iceState.puck.vx * iceState.puck.vx + iceState.puck.vy * iceState.puck.vy);
            const isSlowing = speed < 1.0 && speed > 0.05 && iceState.gameState === 'sliding';
            const isFinished = iceState.gameState === 'finished';

            if (isSlowing || isFinished) {
                const px = iceState.puck.x * s;
                const py = iceState.puck.y * s;
                iceZoomTarget = 1.8;
                iceZoomX = (px - w / 2) * 0.3;
                iceZoomY = (py - w / 2) * 0.3;
            } else {
            ...
            iceZoom += (iceZoomTarget - iceZoom) * 0.08;
            iceZoomX += (0 - iceZoomX) * 0.06;
            iceZoomY += (0 - iceZoomY) * 0.06;

            iceCtx.save();
            iceCtx.translate(w / 2, w / 2);
            iceCtx.scale(iceZoom, iceZoom);
            iceCtx.translate(-w / 2 + iceZoomX, -w / 2 + iceZoomY);
            ...
            if (pts.length === 0) { iceCtx.restore(); return; }
            ...
            iceState.players.forEach(p => {
                const x1 = p.segStart * s,
                    x2 = p.segEnd * s;
                const segW = Math.max(x2 - x1, 0.5);
                const isWinner = iceState.gameState === 'finished' && iceState.winnerId === p.id;
                const segGrad = iceCtx.createLinearGradient(0, 0, 0, w);
                segGrad.addColorStop(0, lightenColor(p.color, isWinner ? 70 : 40));
                segGrad.addColorStop(1, p.color);
                iceCtx.globalAlpha = isWinner ? 1 : 0.93;
                iceCtx.fillStyle = segGrad;
                iceCtx.fillRect(x1, 0, segW, w);
                ...
                if (isWinner) {
                    ...
                    iceCtx.strokeRect(x1 + 1.5, 1.5, segW - 3, w - 3);
                    iceCtx.restore();
                }
                ...
                if (segW > 30) {
                    const avatarR = Math.min(segW * 0.32, 18);
                    const avatarY = half - 14;
                    iceCtx.save();
                    iceCtx.beginPath();
                    iceCtx.arc(segCenterX, avatarY, avatarR, 0, Math.PI * 2);
                    iceCtx.closePath();
                    iceCtx.clip();
                    if (p.pfpImg && p.pfpImg.complete && p.pfpImg.naturalWidth > 0) {
                        iceCtx.drawImage(p.pfpImg, segCenterX - avatarR, avatarY - avatarR, avatarR * 2, avatarR * 2);
                    } else {
                        iceCtx.fillStyle = p.color || '#555';
                        iceCtx.fillRect(segCenterX - avatarR, avatarY - avatarR, avatarR * 2, avatarR * 2);
                    }
                    iceCtx.restore();
                    ...
                    if (segW > 44) {
                        ...
                        iceCtx.fillText(p.bet, segCenterX, avatarY + avatarR + 14);
                    }
                }
            });
            iceCtx.restore();
            ...
            if (iceState.gameState === 'spinning' && iceState.spinStartTime) {
                ...
                const ringR = Math.max(w * 0.08, 18);
                iceCtx.save();
                iceCtx.strokeStyle = 'rgba(220,235,255,0.85)';
                iceCtx.lineWidth = 3;
                iceCtx.beginPath();
                iceCtx.arc(half, half, ringR, 0, Math.PI * 2);
                iceCtx.stroke();
                iceCtx.lineWidth = 2;
                iceCtx.beginPath();
                iceCtx.moveTo(half - ringR * 0.3, half);
                iceCtx.lineTo(half + ringR * 0.3, half);
                iceCtx.moveTo(half, half - ringR * 0.3);
                iceCtx.lineTo(half, half + ringR * 0.3);
                iceCtx.stroke();
                iceCtx.restore();
                if (t < 1) drawIceArrow(iceCtx, half, half, angle, Math.max(ringR * 1.2, 24));
            }

            // ─── Puck (outline + crosshair) with pop ──────────────
            if (iceState.gameState === 'sliding' || iceState.gameState === 'finished') {
                const px = iceState.puck.x * s,
                    py = iceState.puck.y * s;
                const r = PUCK_RENDER_RADIUS * s;
                let popScale = 1;
                if (iceState.gameState === 'sliding') {
                    const elapsedSinceLaunch = (Date.now() - iceState.launchTime) / 1000;
                    if (elapsedSinceLaunch < 0.3) {
                        popScale = Math.min(1, elapsedSinceLaunch / 0.3);
                        const c1 = 1.70158;
                        const c3 = c1 + 1;
                        popScale = 1 + c3 * Math.pow(popScale - 1, 3) + c1 * Math.pow(popScale - 1, 2);
                    }
                }
                const drawR = r * popScale;
                if (drawR > 0.5) {
                    iceCtx.save();
                    ...
                    iceCtx.beginPath();
                    iceCtx.arc(px, py, drawR, 0, Math.PI * 2);
                    iceCtx.stroke();
                    const crossSize = drawR * 0.6;
                    iceCtx.lineWidth = 2;
                    iceCtx.strokeStyle = '#eaf6ff';
                    iceCtx.shadowBlur = 0;
                    iceCtx.beginPath();
