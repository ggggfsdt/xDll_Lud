...
function resetGame() {
    game.state = 'idle';
    // Keep players but reset their bet and position
        ...
        try {
            ...
            switch (data.type) {
                case 'join': {
                    ...
                    let existing = game.players.find(p => p.id === ws.id);
                    if (existing) {
                        // Re-join
                        existing.alive = true;
                        existing.bet = 0;
                        ...
                        existing.vy = 0;
                        existing.radius = 18;
                        existing.mass = 18 * 18 * 1.2;
                        ws.send(JSON.stringify({ type: 'joined', id: existing.id }));
                        broadcastState();
                        break;
                    }
                ...
                case 'bet': {
                    ...
                    game.players.forEach(p => {
                        ...
                        p.popStartTime = Date.now(); // trigger pop on bet increase
                        p.popDuration = 400;
                    });
                    broadcastState();
