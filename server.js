...
// ═══════════════════════════════════════════════════════════════
// ICE ARENA — BSP LAYOUT + RANDOM SHAPES (NO GAPS)
// ═══════════════════════════════════════════════════════════════
...
function makeIcePlayer(id, bet, name, pfp) {
  ...
  // 50% rect, 50% triangle (diagonal split)
...
io.on('connection', (socket) => {
  ...
  socket.on('join', async ({ initData }, ack) => {
    try {
      ...
      // Send current ice room players so the client can restore bets immediately
      ...
      ack?.({
        ...
        icePlayers: icePlayers, // <-- send current ice players
        icePot: iceRoom.pot,    // <-- send current pot
      });
      broadcastState();
    } catch (err) {
...
// ADMIN API (unchanged)
// ─────────────────────────────────────────────────────────────
// ... (keep your existing admin endpoints) ...
